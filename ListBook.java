...
public class ListBook {
	...
	private void getBooks() {
		 DefaultTableModel model = (DefaultTableModel) table.getModel();
		 model.setRowCount(0);
	     Object rowData[] = new Object[10];System.out.println(list.size());
		 for(int i = 0; i < list.size(); i++)
	        { model=add_book_to_table(model,rowData,list.get(i));} 
	}
    ...
    public void get_av(JCheckBox c) {
    	...
    	if(c.isSelected()) {
    		for(int i=0; i<list.size();i++) {
    			if(list.get(i).isAvailable()) {System.out.println(i);list.remove(i);}
    		}
    	} else {
    		for(int i=0; i<list.size();i++) {
    			if(!list.get(i).isAvailable()) {list.add(list.get(i));}
    		}
    	}
    	getBooks();
    }
...
panel.add(edate);
JLabel l7 = new JLabel("Date d'édition");
