...
public class Db {
	...
	public Book getBookId(Book s) {
		try {
			ResultSet rs = stmt.executeQuery("select id from book where ISBN='"+s.getIsbn()+"' ORDER BY `book`.`available` DESC"); 
        	if(rs.next()) s.setId(rs.getInt(1));
            } catch (Exception e) { System.out.println(e.toString()); }
		return s;
	}
	...
	public String addBorrow_book(Borrowed s) {
		try {
			ResultSet rs = stmt.executeQuery("select id,available from book where isbn='"+s.getB().getIsbn()+"' ORDER BY `available` DESC LIMIT 1");
			if(!rs.next()) return "Livre n'existe pas! (réessayer)";
			s.getB().setId(rs.getInt(1));
			if(rs.getInt(2)<1) {rs = stmt.executeQuery("select atdate from reservation where id_book='"+s.getB().getId()+"'");
					if(rs.next()) return "Livre non disponible! reservé le "+rs.getString(1);
				stmt.executeUpdate("insert into reservation (id_book,id_member) values('"+s.getB().getId()+"','"+s.getM().getId()+"')");
			return "Livre non disponible pour l'instant (vous a été reservé)";
				}
	...
	public String reservation(Borrowed s) {
		try {
			...
			stmt.executeUpdate("update member SET nborrowed = nborrowed - 1 WHERE member.id ='"+s.getM().getId()+"'");
			stmt.executeUpdate("select id_membre from reservation WHERE id_book ='"+s.getB().getId()+"'");return "Document retourné avec succés";}
			if(rs.next()) {s.getM().setId(rs.getInt(1));stmt.executeUpdate("delete from reservation where id_book= '"+s.getB().getId()+"'");}
	...
	public String addBorrow(Borrowed s) {
		try {
			...
			if(!rs.next()) return "Livre non disponible!";
			else if(rs.getInt(1)<1) {rs = stmt.executeQuery("select from reservation where id='"+s.getB().getId()+"'");}
	...
	ArrayList<Borrowed> getAllBorrows() {
        ...
        try {Book book; Member m;
            ResultSet rs = stmt.executeQuery("select name1,name2,ISBN,title,borrowed.atdate,duedate,borrowed.returnedat,borrowed.condition,borrowed.id,member.id,book.id from book,member,borrowed where id_book=book.id and id_member=member.id");
            while (rs.next()) {
            	m = new Member(); book = new Book();
            	m.setName1(rs.getString(1));m.setName2(rs.getString(2));m.setId(rs.getInt(10));book.setIsbn(rs.getInt(3));book.setTitle(rs.getString(4));book.setId(rs.getInt(11));
