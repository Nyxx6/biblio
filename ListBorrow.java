...
public class ListBorrow {
	...
	private void selectRow() {                                     
        ...
        if(index<0 || list.get(index).getCondition() == null) {
        frame.dispose();
		...
		if(list.get(index).getB().getId() < 1) {
			rb.r.setSelected(true);rb.isbn.setEnabled(true);rb.id.setEnabled(false);
		rb.id.setText("");
		rb.isbn.setText(model.getValueAt(index, 2).toString());} 
		else {
			rb.r1.setSelected(true);;rb.id.setEnabled(true);rb.isbn.setEnabled(false);
			rb.isbn.setText("");rb.id.setText(Integer.toString(list.get(index).getB().getId()));}
		}
	...
	private void initialize() {
		frame = new JFrame();
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(ListBorrow.class.getResource("/Pack/logo.png")));
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"Nom", "Prénom", "ISBN/ID", "Titre du document", "Date d'emprunt", "Date retour emprunt", "Retourné le:", "Condition de retour"
			}
		){
